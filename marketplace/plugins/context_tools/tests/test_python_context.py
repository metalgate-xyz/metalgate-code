...
class TestCodeCacheRootKeying:
    """The definitions and symbols tables are keyed by the LSP root, so a
    result cached under one root is invisible after the server is steered
    to another."""

    def test_definition_is_keyed_by_root(self, tmp_path):
        from marketplace.plugins.context_tools.extension.cache import (
            _CACHE_MISS,
            CodeCache,
        )
        ...
        Path(file).write_text("def foo():\n    pass\n")

        # Store a definition under root A, then ask for it under root B.
        cache.set_definition("/root/A", file, 1, "foo", {"name": "foo"})
        assert cache.get_definition("/root/A", file, 1, "foo") == {"name": "foo"}
        assert cache.get_definition("/root/B", file, 1, "foo") is _CACHE_MISS

    def test_symbol_is_keyed_by_root(self, tmp_path):
        from marketplace.plugins.context_tools.extension.cache import CodeCache
    ...
    def test_symbol_miss_returns_none(self, tmp_path):
        ...
        cache = CodeCache(str(tmp_path / "cache.db"))
        assert cache.get_symbol("/root/A", "absent") is None
...
# relative paths
#
# The agent frequently passes a bare filename ("orders.py") or a project-rooted
# relative path.  Every public tool must resolve such inputs against the
# language-server root (self.root), NOT the process CWD — otherwise the read
# raises FileNotFoundError before any LSP call, and URIs/cache keys are built
# from a broken path.  These tests pass a bare filename and assert the result
# equals the absolute-path result, proving the resolution is anchored at the
# root and produces one canonical (absolute) path string in returned dicts.
class TestRelativePaths:
    """Bare-filename inputs resolve against the project root, not the CWD.

    The fix lives on the tracer (``_resolve_path``), so these tests exercise
    it through the agent-facing closures.  CWD is deliberately left as-is:
    a correct fix uses ``self.root`` and is independent of the CWD, while a
    broken fix (CWD-relative join) would fail because the pytest CWD is the
    repo root, not the sample directory.
    """

    @staticmethod
    def _normalize_outline(outline):
        # Strip the "file" field for comparison — the absolute and relative
        # calls must produce the same absolute path, but comparing the whole
        # dicts (including "file") is the real test.
        return outline

    def test_get_file_outline_bare_filename_matches_absolute(self, tools):
        abs_result = tools["get_file_outline"](ORDERS_FILE)
        rel_result = tools["get_file_outline"](Path(ORDERS_FILE).name)
        assert rel_result == abs_result, (
            "relative-path outline differs from absolute-path outline"
        )
        # The returned file paths must be absolute (canonical), not bare.
        for s in rel_result:
            assert os.path.isabs(s["file"]), (
                f"expected absolute file path, got {s['file']!r}"
            )

    def test_get_source_bare_filename_matches_absolute(self, tools):
        symbols = tools["get_file_outline"](VALIDATION_FILE)
        line = next(s for s in symbols if s["name"] == "validate_address")["line"]
        abs_result = tools["get_source"](VALIDATION_FILE, line)
        rel_result = tools["get_source"](Path(VALIDATION_FILE).name, line)
        assert rel_result == abs_result, (
            "relative-path get_source differs from absolute-path get_source"
        )
        assert os.path.isabs(rel_result["file"]), (
            f"expected absolute file path, got {rel_result['file']!r}"
        )

    def test_goto_definition_bare_filename_matches_absolute(self, tools):
        # Line 14: if not validate_address(self.address)
        abs_result = tools["goto_definition"](ORDERS_FILE, 14, "validate_address")
        rel_result = tools["goto_definition"](
            Path(ORDERS_FILE).name, 14, "validate_address"
        )
        assert rel_result == abs_result, (
            "relative-path goto_definition differs from absolute-path goto_definition"
        )
        if rel_result.get("file"):
            assert os.path.isabs(rel_result["file"]), (
                f"expected absolute file path, got {rel_result['file']!r}"
            )

    def test_get_callers_bare_filename_matches_absolute(self, tools):
        symbols = tools["get_file_outline"](VALIDATION_FILE)
        line = next(s for s in symbols if s["name"] == "validate_address")["line"]
        abs_result = tools["get_callers"](VALIDATION_FILE, line)
        rel_result = tools["get_callers"](Path(VALIDATION_FILE).name, line)
        assert rel_result == abs_result, (
            "relative-path get_callers differs from absolute-path get_callers"
        )
        for c in rel_result:
            assert os.path.isabs(c["file"]), (
                f"expected absolute file path, got {c['file']!r}"
            )

    def test_get_callees_bare_filename_matches_absolute(self, tools):
        symbols = tools["get_file_outline"](ORDERS_FILE)
        line = next(s for s in symbols if s["name"] == "process")["line"]
        abs_result = tools["get_callees"](ORDERS_FILE, line)
        rel_result = tools["get_callees"](Path(ORDERS_FILE).name, line)
        assert rel_result == abs_result, (
            "relative-path get_callees differs from absolute-path get_callees"
        )
        for c in rel_result:
            assert os.path.isabs(c["file"]), (
                f"expected absolute file path, got {c['file']!r}"
    ...
    def test_relative_path_does_not_raise(self, tools):
        """Regression: a bare filename must not raise FileNotFoundError.

        Before the fix, the first action of every tool read the file with a
        relative path resolved against the process CWD, raising before any
        LSP call.  Each tool must now return its normal (possibly empty)
