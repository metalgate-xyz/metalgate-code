...
_CALLERS_WORKERS = int(os.environ.get("CALLERS_WORKERS", "4"))
...
def _name_col_on_line(line_text: str, name: str, occurrence: int = 0) -> int | None:
    ...
    """
    for idx, m in enumerate(re.finditer(rf"\b{re.escape(name)}\b", line_text)):
        if idx == occurrence:
            return m.start()
    return None
...
def _path_to_uri(path: str) -> str:
    ...
    Does NOT resolve symlinks — exact string matching of the raw path is
    used by callers (and would break on macOS where /tmp → /private/tmp).
    """
...
def _ts_find_function_and_calls(
    config: TreeSitterConfig, source_bytes: bytes, line: int
) -> tuple[int, int, str | None, list[tuple[int, int]]] | None:
...
class Tracer(ABC):
    ...
    def __init__(
        ...
        # Do NOT resolve symlinks — exact string matching of the raw path is
        # used by callers (and would break on macOS where /tmp → /private/tmp).
        self.root = Path(root)
        self.cache = cache
    ...
    def _read_file(self, file: str, limit: int = 10000) -> str:
        """Read file content directly from the host filesystem."""
        return Path(file).read_text(encoding="utf-8", errors="ignore")

    def _read_file_bytes(self, file: str, limit: int = 10000) -> bytes:
        """Read file content as bytes directly from the host filesystem."""
        return self._read_file(file, limit=limit).encode("utf-8", errors="ignore")

    def _resolve_path(self, file: str) -> str:
        """Resolve *file* to an absolute, lexically-normalized path.

        Relative paths anchor against the language-server root (``self.root``)
        rather than the process CWD.  pathlib's ``/`` discards the base when
        *file* is already absolute, so no branch is needed for that case.
        ``Path.absolute()`` makes the result absolute without touching the
        filesystem, and ``os.path.normpath`` collapses ``.``/``..``/``//``
        lexically so reads, URIs, cache keys, and the ``file`` field in
        returned dicts all agree on one canonical string.  Symlinks are NOT
        resolved — exact string matching is required (see ``_path_to_uri``;
        resolving would break on macOS where /tmp → /private/tmp).
        """
        return os.path.normpath((self.root / file).absolute())

    def stop(self) -> None:
        """Tear down the language server, if one has been started.

        Safe to call when no server was started (e.g. only tree-sitter
        operations ran).  Subclasses override this to close their LSP client.
        """
        return

    def set_root(self, root: str) -> None:
        """Point the language server at a different project root.

        Stops the running language server (if any) and clears the set
        of opened documents; the next LSP request lazily rebuilds the
        server against *root*.  Use this when the agent has determined
        the real root of a module/package/library differs from the
        session's working directory — the language server only indexes
        files under its root, so steering it at the correct directory is
        required for workspace-wide queries (e.g. symbol search) to find
        symbols that live outside the default root.
        """
        self.stop()
    ...
    def _extract_def_info(self, d: dict) -> tuple[str, int, int, str] | None:
    ...
    def _uri_to_result_path(self, uri: str) -> str:
        """Convert an LSP ``file://`` URI to a host path.

        The LSP server runs on the host and returns host paths in its URIs;
        this default implementation just strips the ``file://`` scheme and
        percent-decodes the remainder.

        Subclasses override this when the LSP server uses a different
        coordinate system (e.g. runs inside a sandbox and returns guest
        paths that need translation back to host paths).
        """
        return _uri_to_path(uri)

    def _first_name_on_line(self, file: str, line: int) -> str | None:
