...
``definitions`` and ``symbols`` are keyed by the language-server root
because their results depend on what the LSP indexes, which in turn
depends on the workspace root.  Steering the root (see
``Tracer.set_root``) therefore produces clean cache misses instead of
serving answers resolved under a previous root — and the persisted
``root`` column makes stale entries visible when inspecting the DB.
"""
...
class CodeCache:
    ...
    def get_definition(
        self, root: str, file: str, line: int, name: str
    ) -> Any | None:
