...
LSP communication is handled by :class:`~.ty_lsp_client.TyLspClient`, which
runs ``ty server`` as a local subprocess against the project root.  File
access is direct on the host filesystem.
...
from .cache import _CACHE_MISS, CodeCache
...
from .ty_lsp_client import TyLspClient
    ...
    for child in body.children:
        if child.type == "expression_statement":
            ...
            if any(s.type == "string" for s in child.children):
                continue
            # Ellipsis (``...``) is a stub body; any other expression is concrete.
            return any(s.type == "ellipsis" for s in child.children)
...
def _ts_find_scope_at_line(source_bytes: bytes, line: int) -> tuple[int, int] | None:
...
class PythonTracer(Tracer):
    ...
    def __init__(
        ...
        *,
        ...
        self._lsp: TyLspClient | None = None
    ...
    # LSP path helpers
    #
    # ty runs on the host as a local subprocess and returns host paths in
    # its URIs, so paths pass through unchanged.

    def _to_host_uri(self, file: str) -> str:
        """Create a ``file://`` URI using the host path (for ty)."""
        return _path_to_uri(file)

    def _uri_to_result_path(self, uri: str) -> str:
        """Convert a ty ``file://`` URI to a host path for the agent."""
        return _uri_to_path(uri)

    def stop(self) -> None:
        """Shut down the ty language server, if one was started."""
        lsp = self._lsp
        if lsp is not None:
            try:
                lsp.stop()
            except Exception:
                logger.warning("ty LSP stop failed", exc_info=True)
            self._lsp = None
    ...
    def goto_definition(
        ...
        if result is not None:
            # Don't cache degraded results from an unhealthy LSP.  ty returns
            # the import-binding line (not the definition) and leaves kind
            # "unknown" / signature empty when it can't resolve properly;
            # caching that would pin the wrong answer until the file edits.
            kind = result.get("kind", "unknown")
            sig = result.get("signature", "")
            if not result.get("error") and not (kind == "unknown" and not sig):
    ...
    def get_callers(self, file: str, line: int) -> list[dict]:
        ...
        try:
            ...
            logger.warning(
                "ty unavailable for get_callers %s:%d", file, line, exc_info=True
            )
            return []
        ...
        for r in refs:
            ...
            if not ref_uri:
                continue
            ref_file = self._uri_to_result_path(_uri_to_path(ref_uri))
    ...
    def get_callees(self, file: str, line: int) -> list[dict]:
        ...
        try:
            ...
            logger.warning(
                "ty unavailable for get_callees %s:%d", file, line, exc_info=True
            )
            return []
        uri = self._to_host_uri(file)
        results: list[dict] = []
    ...
    def _dedup_callees(self, results: list[dict]) -> list[dict]:
        ...
        for r in results:
            ...
            if (
                existing is None
                or self._callee_is_stub(existing)
                and not self._callee_is_stub(r)
            ):
                by_name[rname] = r
    ...
    def find_symbol(self, name: str) -> list[dict]:
        ...
        try:
            ...
            for sym in symbols:
                ...
                if not sym_uri:
                    continue
                sym_file = self._uri_to_result_path(_uri_to_path(sym_uri))
        ...
        if not results:
            ...
            if lsp_error:
                entry["error"] = lsp_error
            return [entry]

        # Cache real (non-empty) hits only; the "no symbols" hint entry
        # carries transient error context and shouldn't be pinned.
    ...
    def _get_lsp(self) -> TyLspClient:
        ...
        with self._lsp_lock:
            ...
            try:
                ...
                logger.warning(
                    "ty failed to start (will retry next call)", exc_info=True
                )
                raise
            self._lsp = lsp
            return self._lsp
