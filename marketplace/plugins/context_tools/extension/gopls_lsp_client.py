...
class GoplsLspClient(LspBaseClient):
    ...
    def __init__(self, root_uri: str, *, cwd: str | None = None) -> None:
    ...
    async def _start_server(self) -> None:
        # ``shutil.which`` walks PATH with ``os.access``/``os.stat``, which the
        # event-loop blocking guard rejects on the guarded background loop this
        # coroutine runs on.  Resolve the binary off-loop; raise the same
        # FileNotFoundError the caller expects when gopls is absent.
        gopls_bin = await asyncio.to_thread(shutil.which, "gopls")
        ...
        #
        # The isdir check calls os.stat, which the event-loop blocking guard
        # rejects when run on the (guarded) background loop this coroutine
        # executes on.  Offload it to a worker thread, the same way the ty
        # client offloads filesystem checks (see _fs_write).
        def _resolve_tmpdir() -> dict[str, str]:
            env = os.environ.copy()
        ...
        env = await asyncio.to_thread(_resolve_tmpdir)

        cmd = [gopls_bin]
        log_file = server_log_path(self._root_uri, "gopls")
        if log_file is not None:
            cmd += ["-logfile", log_file, "-rpc.trace"]
        cmd.append("serve")
    ...
    async def _read_raw(self) -> bytes | None:
        ...
        try:
            ...
            logger.debug("error reading gopls stdout", exc_info=True)
            return None
        if not data:
            return None
        return data
