...
def server_log_path(root_uri: str, server_name: str) -> str | None:
    """Resolve a file path for a language server's log output, or None.

    Enabled by setting the ``METALGATE_LSP_LOG`` environment variable to a
    truthy value. When enabled, the server writes to
    ``<project_root>/.metalgate/logs/<server>.log``. When unset, no log
    file is configured and the server keeps its default behavior.
    """
    if not os.environ.get("METALGATE_LSP_LOG"):
        return None
    root_path = root_uri.replace("file://", "")
    log_dir = os.path.join(root_path, ".metalgate", "logs")
    # Creating the directory touches the filesystem; run it in a worker
    # thread so it never executes on a guarded event loop.
    import concurrent.futures

    with concurrent.futures.ThreadPoolExecutor(max_workers=1) as ex:
        ex.submit(os.makedirs, log_dir, exist_ok=True).result()
    return os.path.join(log_dir, f"{server_name}.log")
...
class LspBaseClient:
    ...
    async def _start(self) -> None:
        ...
        if self._reader_task is not None and not self._reader_task.done():
            ...
            try:
                ...
                logger.debug("suppressed error cancelling reader task", exc_info=True)
        ...
        # Send both `rootUri` (single-root, LSP < 3.6 servers) and
        # `workspaceFolders` (array form, LSP 3.6+).  ty ignores
        # `rootUri` and falls back to its own working directory when
        # `workspaceFolders` is absent, indexing the wrong tree.
        # gopls honors `rootUri` but also accepts `workspaceFolders`.
        root_path = self._root_uri.removeprefix("file://")
        ...
        try:
            ...
            # The process was spawned by _start_server() above.  If the
            # handshake fails (timeout, server error), tear it down so the
            # failed boot leaves no orphaned server process — the caller
            # (tracer._get_lsp) drops the client and retries with a fresh
            # one on the next request.  Re-raise so the caller sees the
            # original failure.
            await self._stop()
            raise
    ...
    def stop(self) -> None:
        ...
        try:
            ...
            logger.debug("error during LSP server stop", exc_info=True)
        self._bg.stop()
    ...
    async def _send_notify(self, method: str, params: dict | None = None) -> None:
