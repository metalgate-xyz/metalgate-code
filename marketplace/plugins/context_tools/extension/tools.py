...
def make_tools(tracer: Tracer) -> list:
    ...
    def set_language_server_root(root: str) -> dict:
        """
        Point the language server at a different project root directory.

        The language server only indexes source files under its root for
        workspace-wide queries like ``find_symbol``.  When the symbols you
        need live in a module, package, or library whose root differs from
        the session's working directory, call this first to steer the
        server at the correct directory, then re-run ``find_symbol``.

        Re-setting the root — even to the same value — tears down and reboots
        the language server, forcing it to re-read workspace configuration
        (e.g. a Go ``go.work`` file).  This is how to pick up external
        ``go.work`` or module changes without manually killing the server
        process.  If per-file queries return ``no views`` after such a
        change, re-set the root (or the correct module root) to rebuild the
        view.

        This stops the running language server and clears its open-document
        state; the server is rebuilt against *root* on the next request.
        Per-file operations (``goto_definition``, ``get_file_outline``) are
        unaffected and keep working immediately.

        Determine the real root yourself before calling: for a Python
        package, it is the directory containing the topmost ``__init__.py``
        (or the dir above it, the one that makes the import path resolve);
        for a Go module, the directory containing ``go.mod``; for a single
        script, the script's own directory.

        Args:
            root: Absolute path to the directory to use as the language
                  server's project root.

        Returns a dict with keys:
            root: the new root path
            previous: the root path that was in effect before

        Example:
            set_language_server_root("/Users/me/code/metalgate/marketplace/plugins/context_tools/extension")
            # → {"root": "/Users/me/code/.../extension",
            #    "previous": "/Users/me/code/metalgate"}
        """
        previous = str(tracer.root)
        tracer.set_root(root)
        return {"root": str(tracer.root), "previous": previous}
