...
- **`evroc-dcode`** — a pip-installed model provider package (`ChatOpenAI` subclass) wired through `config.toml` as an arbitrary provider via `class_path`. Models are fetched dynamically from the evroc API at launch and written to `evroc-dcode/data/_profiles.py` so the `/model` switcher is populated.
...
### Model provider (`evroc-dcode` package)

The evroc endpoint (`https://models.think.cloud.evroc.com/v1`) speaks the OpenAI Chat Completions API. `evroc-dcode` is a pip-installable package providing `ChatModel` — a thin `ChatOpenAI` subclass. `config.toml` registers it as an arbitrary provider:
...
dcode imports `ChatModel` via `importlib.import_module("evroc_dcode")` and instantiates it with `model=`, `base_url=`, `api_key=`, and any `params` from the config table.

### Model discovery (`_profiles.py`)

dcode discovers models for `class_path` providers by reading `<package>.data._profiles` — a `_PROFILES` dict bundled inside the package, the same way `langchain_openai/data/_profiles.py` works for the built-in `openai` provider. At launch, `run.sh` calls `evroc-dcode.generate_profiles()` which fetches `GET /models` from the evroc API and writes `evroc-dcode/data/_profiles.py` dynamically.
...
```
pyproject.toml                              # uv project: deepagents-code==0.1.65 + evroc-dcode (path dep)
run.sh                                      # fetches models, generates config.toml, installs plugin, launches dcode
evroc-dcode/                                # pip-installable provider package
...
└── evroc_dcode/
    ...
    ├── chat_model.py                       # ChatModel(ChatOpenAI) — class_path = "evroc_dcode:ChatModel"
