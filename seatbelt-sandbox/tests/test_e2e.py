"""End-to-end: boot dcode under the seatbelt sandbox and verify it fences.

Exercises the full dcode stack via ``run.sh``: the server graph boots, the
seatbelt provider's blocking ``get_or_create`` runs on the langgraph event
...
def test_seatbelt_sandbox_boots_and_fences() -> None:
    ...
    reads a junk marker placed under ``$HOME`` outside the launch dir, which
    seatbelt must fence ("Operation not permitted").
    """
    ...
    try:
        ...
        # The seatbelt fence must block the home marker.
